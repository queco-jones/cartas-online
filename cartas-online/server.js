...
function publicRoom(room){return {code:room.code,hostId:room.hostId,started:room.started,phase:room.phase,judgeId:room.judgeId,blackCard:room.currentBlack?.text||null,cardsRequired:room.currentBlack?.pick||1,deckStats:{black:cards.black.length,white:cards.white.length},roundNumber:room.roundNumber,maxRounds:room.maxRounds,players:room.players.map(p=>({id:p.id,name:p.name,avatar:p.avatar,score:p.score,cardCount:p.hand.length,hasPlayed:room.submissions.some(s=>s.playerId===p.id),surveyDone:Boolean(room.surveys?.[p.id])})),submissions:room.phase==='judging'?room.submissions.map(s=>({submissionId:s.id,cards:s.cards})):[],roundWinnerId:room.roundWinnerId||null,winningCards:room.winningCards||null,vote:publicVote(room),survey:{active:isSurveyRound(room)&&room.phase==='round-survey',requiredIds:surveyRequiredIds(room),submittedIds:Object.keys(room.surveys||{})},gameSummary:room.phase==='game-over'?buildSummary(room):null};}
...
io.on('connection',socket=>{
  ...
  socket.on('choose-winner',({submissionId},cb)=>{const room=getRoom(socket);if(!room||room.phase!=='judging'||room.judgeId!==socket.id)return cb?.({ok:false,error:'No puedes elegir ahora.'});const s=room.submissions.find(x=>x.id===submissionId),w=room.players.find(p=>p.id===s?.playerId);if(!s||!w)return cb?.({ok:false,error:'Respuesta no válida.'});w.score++;s.cards.forEach(c=>count(room.stats.winningWhiteUses,c));room.roundWinnerId=w.id;room.winningCards=s.cards;room.surveys={};room.stats.rounds.push({round:room.roundNumber,black:room.currentBlack.text,winner:w.name,winningCards:[...s.cards]});room.phase=isSurveyRound(room)?'round-survey':'round-result';cb?.({ok:true});emitRoom(room);});
  ...
  socket.on('next-round',(_d,cb)=>{const room=getRoom(socket);if(!room||room.hostId!==socket.id||!['round-result','round-survey'].includes(room.phase))return cb?.({ok:false,error:'No puedes continuar ahora.'});const pending=room.phase==='round-survey'?surveyRequiredIds(room).filter(id=>!room.surveys[id]):[];if(pending.length)return cb?.({ok:false,error:`Faltan ${pending.length} encuesta${pending.length===1?'':'s'} por responder.`});if(room.roundNumber>=room.maxRounds){finishGame(room);return cb?.({ok:true,finished:true});}room.roundNumber++;advanceJudge(room);startRound(room);cb?.({ok:true});emitRoom(room);});
